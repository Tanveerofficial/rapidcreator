...
<script>
    function deleteData(element, end_point_url, id, module_name) {
        ...
        let button_text = '<i class="fas fa-trash fs-6"></i>';
        if (!end_point_url.includes("mockup") && !end_point_url.includes("template")) {
            button_text += ' Delete';
        }
            ...
            if (result.isConfirmed) {
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        } else if (response.message == "template" || response.message == "product") {
                            button.innerHTML=button_text;
                            button.removeAttribute("disabled");
                            Swal.fire({
                                toast: true,
